...
class Doc:
            ...
            for line_idx, orig_p in enumerate(all_p):
                ...
                for p in p_list:
                    if line_idx > len(all_p) - 3:
                        ...
                        if not (
                            text_only == "" and year_pattern.search(p.text) is None
                        ):
                            # Possible year?
                    ...
                    box_style, line_style, word_line_styles = (
                        style_utils.parse_tika_style(p["style"], p.text, page_width)
                    )
    ...
    def visual_lines_to_blocks(
        ...
        while line_idx < len(visual_lines):
            ...
            if is_new_group or is_mixed_font or line_idx == len(visual_lines) - 1:
                ...
                if block_type == "para" and (
                    ...
                    for vl in group_buf[1:]:
                        if not vhu.compare_top(vl, prev_vl):
                            ...
                            result_list[-1].append(vl)
                        prev_vl = vl
                    if len(group_buf) == 1 and group_buf[0]["text"].lower().startswith(
                        "section"
                    ):
                        result_list, buf_texts, block_types = (
                            self.create_new_vl_group_for_sections(
                ...
                for i, grp_buf in enumerate(final_group_bufs):
                    ...
                    if block_types[i] == "header_modified":
                        ...
                        block_modified = True
                        block_class = grp_buf[0]["class"]
                    elif block_types[i] == "header" and group_buf[0][
                        "text"
                    ].lower().startswith("section"):
                        block_class = grp_buf[0]["class"]
                    block = {
                    ...
                    if not prev_block_footer_discarded:
                        if (
                            block["block_type"] in ["para", "header"]
                            and block["visual_lines"][0]["box_style"][0]
                            > page_height - (footer_margin * page_height)
                            and len(block["visual_lines"]) == 1
                            and len(block["visual_lines"][0]["line_parser"]["words"])
                            == 1
                        ):
                            block_to_discard = True
                    elif prev_discarded_block:
                        if (
                            block["visual_lines"][0]["box_style"][0]
                    ...
                    if (
                        len(page_blocks)
                        and block_types[i] == "table_row"
                        and page_blocks[-1]["block_type"] == "para"
                    ):
                        # don't attempt this if the block spans multiple lines
                        prev_multi_line = len(
                            set(
                                [
                                    prev_child_block["box_style"][0]
                                    for prev_child_block in page_blocks[-1][
                                        "visual_lines"
                                    ]
                                ]
                            )
                        ...
                        if prev_multi_line == 1:
                            convert_tr, cells, new_visual_lines = check_possible_table(
                                page_blocks[-1], block
                            )
                            if convert_tr:
                ...
                #  group_buf = [line_info]
                if line_idx == len(visual_lines) - 1 and (
                    not is_new_group or should_ignore
                ):
                    group_buf = []
                else:
                    group_buf = [line_info]
                ...
                if (
                    ...
                    and len(group_buf)
                    and len(page_blocks) > 0
                    and page_blocks[-1]["box_style"][0] > line_info["box_style"][0]
                ):
                    # Create a block anyways here
                    ...
                    block = {
                        ...
                        # 'line_props': line_props,
                        "visual_lines": group_buf,
                        "block_class": Counter(line_info["word_classes"]).most_common()[
                            0
                        ][0],
                        "block_modified": False,
                    }
            ...
            line_idx = line_idx + 1
        if (
            prev_block_footer_discarded
            and len(page_blocks) > 0
            and ends_with_sentence_delimiter_pattern.search(
                page_blocks[-1]["block_text"]
            )
            is None
        ):
    ...
    def detect_new_group(
        ...
        change_in_class, is_new_para, same_top = self.compare_with_previous_line(
            line_info, prev_line_info, group_buf, group_is_table_row
        )

        is_table_row_list, is_table_row = self.detect_table_or_list(
            group_buf,
            group_is_table_row,
            line_info,
            prev_line_info,
        ...
        if not group_is_table_row and is_table_row:
            ...
            if is_new_group and not same_top and not change_in_class:
                ...
                if (
                    line_info["box_style"][0] > prev_line_bottom
                    and (
                        line_info["box_style"][0] - prev_line_bottom
                        <= 1.5 * prev_line_info["box_style"][4]
                    )
                    and (
                        prev_line_info["box_style"][1]
                        <= line_info["box_style"][1]
                        < prev_line_info["box_style"][2]
                        and prev_line_info["box_style"][1]
                        < line_info["box_style"][2]
                        <= prev_line_info["box_style"][2]
                    )
                ):
                    prev_vl_center = (
                        prev_line_info["box_style"][1]
                        + prev_line_info["box_style"][3] / 2
                    )
                    curr_vl_center = (
                        line_info["box_style"][1] + line_info["box_style"][3] / 2
                    )
                    if (
                        abs(curr_vl_center - prev_vl_center) <= 5
                        or prev_line_info["box_style"][1] == line_info["box_style"][1]
                        or line_info["box_style"][2] == prev_line_info["box_style"][2]
                    ):
                        is_new_group = False
        else:
            top_diff = None
            ...
            is_new_group = (
                (
                    ...
                    or (
                        is_list_start
                        and top_diff
                        and line_info["box_style"][4] <= top_diff
                    )
                )
                and not same_top
            ...
            if (
                ...
                and same_top
                and is_new_para
                and line_info["box_style"][1] < prev_line_info["box_style"][2]
            ):
                is_new_group = True
        ...
        is_list_start_separate_line = (
            (
                lp_line.length == 1
                and lp_line.is_list_item
                and not (group_is_table_row and same_top)
            )
            or (
                (lp_line.is_list_item or lp_line.numbered_line)
                and not (group_is_table_row and same_top)
            )
        ...
        if (
            is_list_start_separate_line
            and lp_line.length == 1
            and same_top
            and len(group_buf) > 1
        ):
            gap, n_gap, _, is_justified = self.get_gaps_from_vls(
        ...
        if prev_line_info:
            prev_list_start_separate_line = prev_line_info and (
                (len(prev_line_info["text"]) == 1 and self.is_list_item(prev_line_info))
                or (
                    is_list_start
                    and not same_top
                    and (
                        prev_line_info["lp_line"].is_list_item
                        or prev_line_info["lp_line"].numbered_line
                    )
                )
            )
            out_of_order_list = (
            ...
            out_of_order_list = out_of_order_list or (
                prev_list_start_separate_line
                and not number_in_braces_pattern.sub("", line_info["text"])
            )
        # out_of_order_list = False
        ...
        is_new_group = (
            (is_new_group or is_list_start or out_of_order_list)
        ...
        is_mixed_font = (
            line_info["word_classes"][line_start_idx] != line_info["word_classes"][-1]
        )
    ...
    def count_possible_cells(self, group_buf):
        if (
            group_buf[0]["line_parser"]["numbered_line"]
            or group_buf[0]["line_parser"]["numbered_line"]
        ):
            return 0
        prev_vl = group_buf[0]
        n_cells = 0
    ...
    def detect_table_or_list(
        ...
        if same_top:
            ...
            if not group_is_table_row:
                if gap > normal_gap or (
                    "changed" in prev_line_info and prev_line_info["changed"]
                ):
                    opt_cond = gap < 4 * normal_gap
                    _, page_width, _, _ = self.page_styles[line_info["page_idx"]]
                    if (
                        line_info["line_style"][2] > 9
                        or line_info["box_style"][3] > 0.5 * page_width
                    ):
                        opt_cond = gap < 6 * normal_gap
                    # Make the gap check conditional if we are dealing with numbers.
                    opt_cond = opt_cond and not (
                        line_info["line_parser"]["word_count"] == 1
                        and line_info["line_parser"]["last_word_number"]
                    )
                    if (
                        (
                            group_buf[-1]["line_parser"]["numbered_line"]
                            and not group_buf[-1]["line_parser"]["is_table_row"]
                        )
                        or group_buf[0]["line_parser"]["numbered_line"]
                        or self.is_list_item(group_buf[0])
                    ) and opt_cond:
                        ...
                        if len(group_buf) > 1:
                            if (
                                not vhu.compare_top(group_buf[-1], group_buf[-2])
                            ) and prev_line_info["text"][-1] in ["."]:
                                # We have a multi line data here. If the previous VL is ending with '.', consider
                                # it not a table row.
                                is_table_row = False
                            if is_table_row and not vhu.compare_top(
                                group_buf[0], line_info
                            ):
                                ...
                                if len(
                                    list(
                                        range(
                                            max(
                                                int(buf0_bstyle[1]), int(buf_bstyle[1])
                                            ),
                                            min(int(buf0_bstyle[2]), int(buf_bstyle[2]))
                                            + 1,
                                        )
                                    )
                                ) >= 0.4 * (
                                    int(buf_bstyle[2]) - int(buf_bstyle[1])
                                ) and list(
                                    range(
                                        max(
                                            int(buf0_bstyle[1]), int(prev_buf_bstyle[1])
                                        ),
                                        min(
                                            int(buf0_bstyle[2]), int(prev_buf_bstyle[2])
                                        )
                                        + 1,
                                    )
                                ):
                            ...
                            if (
                                ...
                                and (
                                    ...
                                    == len(line_info["line_parser"]["words"])
                                    == 1
                                )
                                ...
                                if (
                                    ...
                                    not line_info["text"].isalnum()
                                    and len(line_info["text"]) == 1
                                ) or (
                                    not prev_line_info["text"].isalnum()
                                    and len(prev_line_info["text"]) == 1
                                ):
                                    do_continue = False
                                if do_continue:
                                    # Prev vl has same top with the current vl, so we can continue with the group_buf
                                    _, spaces = vhu.get_avg_space_bw_multi_line_vls(
                                        group_buf
                                    )
                                    if len(spaces) > 1 and spaces[-1] <= max(
                                        spaces[:-1]
                                    ):
                        ...
                        if (
                            is_table_row
                            and (
                                group_buf[0]["text"].lower().startswith("section")
                                or group_buf[0]["text"].startswith("Item")
                            )
                            and (
                                (
                                    prev_line_info["text"][-1]
                                    in line_parser.continuing_chars
                                    and (
                                        line_info["text"][-1]
                                        in line_parser.continuing_chars
                                        or (
                                            ...
                                            .lower()
                                            .startswith("section")
                                            and prev_line_info["word_classes"][-1]
                                            == line_info["word_classes"][0]
                                        )
                                    )
                                )
                                or (
                                    group_buf[0]["word_classes"][0]
                                    != line_info["word_classes"][0]
                                )
                            )
                        ):
                            ...
                            is_table_row
                            and group_buf[0]["text"].lower().startswith("section")
                            and section_num_pattern.search(
                                prev_line_info["text"].strip().split()[-1]
                            )
                            is not None
                        ):
                    ...
                    len(page_blocks) > 0
                    and page_blocks[-1]["block_type"] == "table_row"
                    and gap > act_normal_gap
                    and gap > 0.5 * normal_gap
                    and len(page_blocks[-1]["visual_lines"]) >= len(group_buf) + 1
                ):
                    ...
                    do_overlap = False
                    for v_idx, vls in enumerate(
                        page_blocks[-1]["visual_lines"][: len(g_buf_vls)]
                    ):
                        if v_idx < len(g_buf_vls) - 1:
                            if (
                                g_buf_vls[v_idx]["box_style"][2]
                                > page_blocks[-1]["visual_lines"][v_idx + 1][
                                    "box_style"
                                ][1]
                            ):
                                do_overlap = True
                    ...
                    if is_table_row and (
                        ...
                        last_blk_bottom = (
                            page_blocks[-1]["box_style"][0]
                            + page_blocks[-1]["box_style"][4]
                        )
                        if (
                            line_info["box_style"][0] - last_blk_bottom
                        ) > 1.5 * page_blocks[-1]["box_style"][4]:
                            is_table_row = False
                            is_list = True
            if LINE_DEBUG:
                print(
                    f"gap: {gap}, normal: {normal_gap}, is_table_cell: {is_table_row} "
                ...
                print(
                    ...
                    line_info["text"][0:40],
                    "prev:",
                    prev_line_info["text"][0:40],
                )
        # group_buf.append(line_info)
        return is_list, is_table_row

    def compare_with_previous_line(
        self, line_info, prev_line_info, block_buf, group_is_table_row
    ):
        ...
        if prev_line_info:
            prev_line_ends_with_line_delim = (
                ends_with_sentence_delimiter_pattern.search(prev_line_info["text"])
                is not None
            )
        if prev_line_info and prev_line_info["page_idx"] == line_info["page_idx"]:
            line_info["space"] = round(
                line_info["box_style"][0] - prev_line_info["box_style"][0], 1
            )
            ...
            same_left_align = (
                abs(line_info["box_style"][1] - prev_line_info["box_style"][1]) < 5
            )
            gap_bw_prev_line = line_info["box_style"][0] - (
                prev_line_info["box_style"][0] + prev_line_info["box_style"][4]
            )
            if not same_top and (
                (line_info["space"] < 0 and prev_line_info["line_style"] != line_style)
                or (group_is_table_row and prev_line_info["lp_line"].word_count < 10)
            ):
                ...
                for vl in block_buf[-1::-1]:
                    ...
                    gap_bw_lines = (
                        line_info["box_style"][0] + line_info["box_style"][4]
                    ) - vl["box_style"][0]
                    if (
                        top_diff <= 2
                        or (
                        ...
                        same_top = vhu.compare_top(line_info, vl) or (
                            (vl["box_style"][4] <= line_info["box_style"][4])
                            and (
                                (
                                    line_info["space"] >= 0
                                    and len(
                                        list(
                                            range(
                                                max(
                                                    int(vl["box_style"][1]),
                                                    int(line_info["box_style"][1]),
                                                ),
                                                min(
                                                    int(vl["box_style"][2]),
                                                    int(line_info["box_style"][2]),
                                                )
                                                + 1,
                                            )
                                        )
                        ...
                        group_is_table_row and same_left_align and gap_bw_prev_line > 0
                    ):
                        if (
                            vl["box_style"][1] - line_info["box_style"][1] < -5
                            and line_info["box_style"][0]
                            - (vl["box_style"][0] + vl["box_style"][4])
            ...
            if (
                max_top > prev_line_info["box_style"][0]
                and max_top_vl["page_idx"] == prev_line_info["page_idx"]
            ):
            ...
            super_script_height = (
                0.10 * prev_line_info["box_style"][4]
                < top_difference
            ...
            if "is_superscript" in prev_line_info:
                follows_superscript = (
                    1.0 * prev_line_info["box_style"][4]
                    < top_difference
                    < 1.1 * prev_line_info["box_style"][4]
                )
        ...
        if prev_line_info:
            ...
            if (
                change_in_class
                and len(prev_line_info["word_classes"]) == 1
                and len(block_buf) > 1
            ):
                grp_buf_word_classes = [
                    wc for li in block_buf for wc in li["word_classes"]
                ]
                ...
                if len(block_buf) == len(grp_buf_word_classes):
                    ...
                    if len(w_class_to_check) == 1:
                        ...
                        cond_for_single_word_vls = (
                            w_class_to_check not in grp_buf_word_classes[-3:]
                        )
                change_in_class = (
                    group_class_name not in w_class_to_check
                ) and cond_for_single_word_vls
            if (
                change_in_class
                and (
                    line_info["page_idx"] != prev_line_info["page_idx"] or not same_top
                )
                and not prev_line_ends_with_line_delim
                and Doc.soft_line_style_check(line_info, prev_line_info)
            ):
                change_in_class = False
            if not change_in_class:
                if line_info["page_idx"] != prev_line_info["page_idx"]:
                    if (
                        prev_line_ends_with_line_delim
                        or line_info["lp_line"].numbered_line
                        or (
                            abs(
                                prev_line_info["box_style"][1]
                                - line_info["box_style"][1]
                            ...
                            < 10
                            and prev_line_info["box_style"][2] < 0.4 * page_width
                        )
                        or (
                            prev_line_info["lp_line"].roman_numbered_line
                            and len(prev_line_info["lp_line"].words) == 1
                        )
                        or self.detect_block_center_aligned(line_info)
                        or self.detect_block_center_aligned(prev_line_info)
                    ):
                        is_new_para = True
                        new_page_para = True
                else:
                    has_more_space = (
                        ...
                        or line_info["space"]
                        > 1.1
                        * self.line_style_space_stats[line_style]["most_frequent_space"]
                        or (line_info["space"] < 0 and not is_superscript)
                    )
                    if (
                        has_more_space
                        and line_style not in self.line_style_space_stats
                        and prev_line_style in self.line_style_space_stats
                    ):
                        ...
                        has_more_space
                        and line_style not in self.line_style_space_stats
                        and prev_line_style not in self.line_style_space_stats
                    ):
                        act_space_bw_lines = line_info["box_style"][0] - (
                            prev_line_info["box_style"][0]
                            + prev_line_info["box_style"][4]
                        )
                        if (
                            0 < act_space_bw_lines < line_style[2]
                            and act_space_bw_lines < prev_line_style[2]
                        ):
                            has_more_space = False
                        elif same_top:
                            has_more_space = False
                    ...
                    if not same_top:
                        ...
                        for blk in block_buf[::-1]:
                            if last_top != blk["box_style"][0]:
                                break
                            left_to_compare_with = min(
                                left_to_compare_with, blk["box_style"][1]
                            )
                    if (
                        left_to_compare_with > half_page_width
                        and box_style[1] > half_page_width
                    ):
                        left_shift_threshold = 80
                    shifts_left = (
                        left_to_compare_with - box_style[1] > left_shift_threshold
                    )
                    ...
                    if (
                        high_top_difference
                        and round(abs(top_difference)) == round(font_size_decider)
                        and abs(top_difference) - font_size_decider <= 0.25
                    ):
                        high_top_difference = False
                    if (
                        not high_top_difference
                        and line_info["lp_line"].numbered_line
                        and prev_line_ends_with_line_delim
                    ):
                        high_top_difference = (
                            abs(top_difference) > line_info["line_style"][2] * 1.5
                        )
                    ...
                    is_new_para = (
                        has_more_space
                        or shifts_left
                        or high_top_difference
                        or (
                            self.detect_block_center_aligned(prev_line_info)
                            and not self.detect_block_center_aligned(line_info)
                        )
                    )
                    if (
                        is_new_para
                        and has_more_space
                        and high_top_difference
                        and line_info["space"] < 0
                        and abs(line_info["space"]) > 0.5 * page_height
                        and not prev_line_ends_with_line_delim
                    ):
                    ...
                    if (
                        not is_new_para
                        and first_vl_numbered_line
                        and line_info["lp_line"].numbered_line
                        and (
                            abs(top_difference)
                            > (line_info["line_style"].font_size * 1.5)
                        )
                    ):
                        is_new_para = True
                    ...
                    if (
                        ...
                        and not same_top
                        and line_info["lp_line"].numbered_line
                        and prev_line_info["lp_line"].numbered_line
                        and prev_line_style == line_info["line_style"]
                    ):
                        is_new_para = True
                    if (
                        is_new_para
                        and not has_more_space
                        and high_top_difference
                        and not prev_line_ends_with_line_delim
                        and abs(
                            prev_line_info["box_style"][1] - line_info["box_style"][1]
                        )
                        < 5
                        and line_info["box_style"][0]
                        - (
                    ...
                    if is_new_para and LINE_DEBUG:
                        print(
                            f"setting new para: space: {has_more_space}, shifts: {shifts_left}, "
                    ...
                    if (
                        ...
                        and not group_is_table_row
                        and not same_top
                        and top_difference < 0
                        and box_style[1] - prev_line_info["box_style"][1]
                        > left_shift_threshold
                    ):
                        is_new_para = True
                    if not is_new_para:
                        ...
                        if (
                            ...
                            buf_text_all_caps = all(
                                [b["text"].isupper() for b in block_buf]
                            )
                            prev_text_all_nouns = all(
                                word["is_noun"]
                                for word in prev_line_info.get(
                                    "line_parser", {"words": []}
                                ).get("words", [])
                            )
                            ...
                            if (
                                buf_text_all_caps
                                and not line_info["text"].isupper()
                                and not prev_text_all_nouns
                            ):
                                is_new_para = True
                ...
                if LINE_DEBUG:
                    print(
                        "old class: ",
                        prev_line_info["word_classes"],
                        prev_line_info["line_style"],
                    )
                    print(
                        "new class: ",
                        line_info["word_classes"],
                        line_info["line_style"],
                    )
            if LINE_DEBUG:
                print(
                    "space: ",
                    round(line_info["space"], 2),
                    "font size: ",
                    line_style[2],
                    f", new para: {is_new_para}",
                    f", new page para: {new_page_para}",
                    f", change_in_class: {change_in_class}" f", same_top: {same_top}",
                )
    ...
    def should_ignore_line(
        ...
        if line_idx > len(all_p) - 2 or line_idx < 2:
            ...
            if line_idx < 2:
                ...
                if (
                    len(remove_whole_numbers) == len(p.text)
                    or not p.text.lower().startswith("page")
                ) and lp_line.word_count > 1:
                    do_ignore = False
            if (
                0 < len(num_only) < 4
                and lp_line.alpha_count < 2
                and not lp_line.dot_numbered_line
                and do_ignore
            ):
                return True, False
            else:
                text_only = text_only_pattern.sub("", p.text).strip()
                if (
                    text_only in last_line_counts
                    and last_line_counts[text_only] > 2
                    and not lp_line.last_word_is_stop_word
                    and line_idx > len(all_p) - 2
                ):
                    return True, False
        if p.text:
            if p.text.startswith("Source:"):
                ...
                not len(single_char_pattern.sub("", p.text).strip())
                and box_style[1] < 5
            ):
        ...
        ignore, ignore_all_after = self.should_ignore(
            p.text,
            "header" if lp_line.is_header else None,
        )
        if ignore:
            return True, False
        if ignore_all_after:
        ...
        if is_page_header and loc_key in page_headers:
            ...
            if len(page_visual_lines) > 0:
                # Sort using top
                sorted_vls = sorted(
                    page_visual_lines, key=lambda vl: vl["box_style"][0]
                )
            ...
            is_page_footer and loc_key in page_footers and not lp_line.dot_numbered_line
        ):
    ...
    def save_file_stats(self):
        ...
        for line_style in keys_by_size:
            ...
            for header_ls in header_block_line_styles:
                if (
                    header_ls != line_style
                    and header_ls[1:] == line_style[1:]
                    and line_style not in new_header_line_styles
                    and self.class_stats.get(self.line_style_classes[line_style], False)
                ):
                    stats = self.class_stats[self.line_style_classes[line_style]]
                    text_with_sentence_delimiter = False
                    for text in stats.get("texts", []):
                        if (
                            ends_with_sentence_delimiter_pattern.search(text)
                            is not None
                        ):
        ...
        mode_font_size = (
            max(set(all_font_sizes), key=all_font_sizes.count)
        ...
        for line_style in keys_by_size:
            ...
            if (line_style[2] > mode_font_size or line_style[3] > 400) or (
                self.line_style_classes[line_style] in self.class_stats
                and self.class_stats[self.line_style_classes[line_style]].get(
                    "n_lines", 0
                )
                < 0.75 * total_num_lines
        ...
        class_by_freq = sorted(
            self.class_stats.items(), key=lambda x: len(x[1]["texts"]), reverse=True
        )
    ...
    def make_header_class(self, line_style):
        if line_style in self.header_styles:
            ...
            cloned_style = LineStyle(
                line_style[0], "italic", line_style[2], 600, "none", "left"
            )
    ...
    def merge_vls_if_needed(self, vls, is_table_row=False):
        ...
        if is_table_row:
            ...
            for vl in vls[1:]:
                if "line_parser" in vl:
                    vls_has_single_words = vls_has_single_words and (
                        len(vl["line_parser"]["words"]) == 1
                    )
        ...
        for vl_id, vl in enumerate(vls[1:]):
            ...
            bottom_gap = (vl["box_style"][0] + vl["box_style"][4]) - (
                prev_vl["box_style"][0] + prev_vl["box_style"][4]
            )
            ...
            should_merge = False
            if (
                prev_vl["text"].strip()[-1] in ["$", "€", "£"]
                and len(prev_vl["text"]) > 1
                and not prev_vl["text"].strip()[-2].isspace()
            ):
                vl["text"] = prev_vl["text"].strip()[-1] + vl["text"]
                prev_vl["text"] = prev_vl["text"].strip()[:-1]
            if prev_vl["text"].strip() in ["$", "€", "£"] and vl["text"].strip()[
                0
            ] not in ["$", "€", "£", "%"]:
                # and vl['line_parser']['words'][0]['is_number']):#sometimes it has _ and other stuff
                should_merge = True
            elif (
                vl["word_classes"][0] != prev_vl["word_classes"][-1]
                and len(vl["text"]) > 1
                and len(prev_vl["text"]) > 1
            ):
                # Difference in class.
                ...
                "line_parser" in vl
                and "line_parser" in prev_vl
                and len(vl["line_parser"]["words"])
                == len(prev_vl["line_parser"]["words"])
                == 1
                and vl["line_parser"]["words"][0].get("is_number", False)
                and prev_vl["line_parser"]["words"][-1].get("is_number", False)
            ):
                ...
                if is_justified:
                    if multi_row:  # Probable center justified table row ?
                        normal_gap = (
                            normal_gap / JUSTIFIED_NORMAL_GAP_MULTIPLIER
                        ) * 2.5
                    else:
                        normal_gap = (
                            normal_gap / JUSTIFIED_NORMAL_GAP_MULTIPLIER
                ...
                if (
                    ...
                    if is_table_row:
                        ...
                        if not multi_row and gap > (cline_style[5] * 3):
                            ...
                            if (
                                prev_vl_merged
                                and avg_merge_gap
                                and gap < 2 * avg_merge_gap
                            ) or vls_has_single_words:
                                should_merge = True
                            ...
                            if vls_has_single_words and vl_id < len(vls) - 2:
                                gap_1, _, _, _ = self.get_gaps_from_vls(
                                    vls[vl_id + 2], vl
                                )
                                if gap < 0.7 * gap_1:
                                    should_merge = False
                            ...
                            if avg_merge_gap and gap > (3 * avg_merge_gap):
                                ...
                                if not multi_row or same_top:
                                    ...
                                    if len(vl_text) > 1 and (
                                        vl_text[0] == "%"
                                        or (
                                            len(prev_vl["line_parser"]["words"]) > 1
                                            and prev_vl["line_parser"]["words"][0].get(
                                                "is_number", False
                                            )
                                            and prev_vl["text"].strip()[-1] == "%"
                                            and vl["line_parser"]["words"][0].get(
                                                "is_number", False
                                            )
                                        )
                                    ):
                                ...
                                if (
                                    ...
                                    and (
                                        len(prev_vl["text"].strip().split()) > 1
                                        or len(vl["text"].strip().split()) > 1
                                    )
                                ):
                                    should_merge = False
                    ...
                    if (
                        should_merge
                        and prev_vl["text"].strip()[-1] in ["$", "€", "£", "%"]
                        and vl["text"].strip()[0] in ["$", "€", "£", "%"]
                    ):
                        should_merge = False
                elif (
                    is_table_row
                    and not same_top
                    and len(gap_bw_vls) > 0
                    and vl["box_style"][0]
                    <= (prev_vl["box_style"][0] + (2 * prev_vl["box_style"][4]))
                    and vl["text"].strip() not in ["$", "€", "£"]
                    and len(
                        list(
                            range(
                                max(
                                    int(prev_vl["box_style"][1]),
                                    int(vl["box_style"][1]),
                                ),
                                min(
                                    int(prev_vl["box_style"][2]),
                                    int(vl["box_style"][2]),
                                )
                                + 1,
                            )
    ...
    def post_fix(self, block):
        if (
            block["block_type"] == "para" or block["block_type"] == "list_item"
        ) and len(block["visual_lines"]) > 0:
            ...
            if word_stats["is_justified"]:
                ...
                for vl in vls[1:]:
                    ...
                    if prev_vl["text"] and prev_vl["text"][-1] == "-" and new_line:
                        ...
                        block_text = (
                            block_text
                            + self.check_add_space_btw_texts(block_text, vl["text"])
                            + vl["text"]
                        )
                    prev_vl = vl
    ...
    def label_table_of_content(self):
        ...
        for idx, block in enumerate(blocks):
            ...
            if block["block_type"] == "table_row" and len(block["visual_lines"]) == 2:
                collected_row.append(
                    {"block_idx": idx, "text": block["visual_lines"][0]["text"]}
                )
            else:
    ...
    def organize_and_indent_blocks(self, debug=False):
        ...
        while idx < len(self.blocks):
            ...
            probable_table_block = (
                False  # self.check_block_within_svg_tags(block, prev_block)
            )
            ...
            if (
                prev_block
                and Doc.have_y_overlap(prev_block, block, check_class=True)
                and not prev_block.get("block_modified", False)
                and not prev_block.get("block_reordered", False)
            ):
                # print(n_table_cols, ">>>>> y overlap: ", prev_block['block_text'], '->', block['block_text'])
                is_y_overlapped = True
                if (
                    block["block_type"] == "table_row"
                    and prev_block["block_type"] != "table_row"
                    and len(block_buf) > 1
                ):
                ...
                if is_y_overlapped:
                    ...
                    if len(organized_blocks) > 1 and not block_buf:
                        ...
                        if (
                            Doc.have_y_overlap(t_block, block, check_class=True)
                            and not t_block.get("block_modified", False)
                            and not t_block.get("block_reordered", False)
                        ):
                            if BLOCK_DEBUG:
                                print(
                                    ">>>>> y overlap t_block: ",
                ...
                if len(block_buf) > 0:
                    ...
                    if BLOCK_DEBUG:
                        print(
                            ...
                            "vl: ",
                            len(new_block["visual_lines"]),
                        )

            block_sents = sent_tokenize(block["block_text"])
            class_name = block["block_class"]

            line_style = block["visual_lines"][-1][
                "line_style"
            ]  # self.class_line_styles[class_name]
            if block["block_type"] == "header":
            ...
            if is_table_row:
                ...
                if (
                    ...
                    for vl in vls[-1:1:-1]:
                        if (
                            abs(box_0[0] - vl["box_style"][0]) < 2
                            or abs(box_1[0] - vl["box_style"][0]) < 2
                        ):
                ...
                if misaligned_top and box_0[1] >= box_1[1]:
                    ...
                    for v_idx in range(len(vls) - 1):
                        if (
                            abs(
                                vls[v_idx]["box_style"][0]
                                - vls[v_idx + 1]["box_style"][0]
                            )
                            > 2
                        ):
                            continue
                    ...
                    if (
                        has_same_top
                        and "changed" in vls[v_idx]
                        and vls[v_idx]["changed"]
                    ):
                        misaligned_top = False
                ...
                if misaligned_top and split_vl["text"][-1] not in [":"]:
                    ...
                    for v_idx, vl in enumerate(vls[1:]):
                        ...
                        if vl_bottom > max_bottom:
                            ...
                            if Doc.check_line_between_box_styles(
                                prev_vl["box_style"], vl["box_style"], svg_page_tags[0]
                            ):
                                split_idx = v_idx + 1
                                break
                        elif (
                            min_top <= vl_bottom <= max_bottom
                            and vl["box_style"][1] > box_0[2]
                        ):
                            misaligned_top = False
                            break
                        else:
                ...
                if (
                    not misaligned_top
                    and orig_misaligned_top != misaligned_top
                    and split_idx > 0
                ):
                    if Doc.check_line_between_box_styles(
                        vls[split_idx - 1]["box_style"],
                ...
                if n_table_cols == 2:
                    ...
                    have_y_overlap = Doc.have_y_overlap(
                        prev_table_row, block, check_class=False
                    )
                    if have_y_overlap:
                        is_table_row = True
                        # print("setting prev table row to ", block["block_text"])
                        # prev_table_row = block
                    aligned_para = table_parser.para_aligned_with_prev_row(
                        self.page_width, prev_table_row, block, debug=False
                    )
            ...
            if n_rows > 2:
                ...
                for tblock in organized_blocks[table_start_idx + 1 : table_end_idx]:
                    temp_diff_in_space = tblock["box_style"][0] - (
                        temp_trow_block["box_style"][0]
                        + temp_trow_block["box_style"][4]
                    )
            ...
            check_again = True
            no_more_non_table_rows = (
                not is_table_row
                and table_start_idx != -1
                and (
                    non_table_row_count > 2
                    or (
                        len(block["block_text"].split()) > 10
                        and different_style
                        and n_rows - non_table_row_count > 1
                    )
                )
                and not aligned_para
                and not have_y_overlap
            )
            if no_more_non_table_rows and non_table_row_count <= 2 and prev_block:
                ...
                if prev_block_table_row:
                    align_count, prev_count, align_pos = (
                        table_parser.get_alignment_count(
                            prev_block, block, force_check_curr_block=True
                        )
                    )
                    if (
                        align_count == len(align_pos)
                        and len(set(align_pos)) == 1
                        and len(block["visual_lines"][0]["text"].split()) < 10
                    ):
                    ...
                    for blk in organized_blocks[-2::-1]:
                        if blk["block_type"] == "table_row":
                            align_count, prev_count, align_pos = (
                                table_parser.get_alignment_count(
                                    blk, block, force_check_curr_block=True
                                )
                            )
                            if (
                                align_count == len(align_pos)
                                ...
                                or (
                                    not align_count
                                    and blk["box_style"][1] > block["box_style"][2]
                                )
                                and len(block["visual_lines"][0]["text"].split()) < 10
                            ):
                                # We are aligned to a single column
            ...
            if (
                ...
                and idx < len(self.blocks) - 1
            ):
                if (
                    self.blocks[idx + 1]["block_type"] == "table_row"
                    and self.blocks[idx + 1]["box_style"][0] >= block["box_style"][0]
                    and (
                        self.blocks[idx + 1]["box_style"][0]
                        + self.blocks[idx + 1]["box_style"][4]
                    )
                    ...
                    for vl in block["visual_lines"][1:]:
                        if (
                            vl["box_style"][0]
                            > vl1["box_style"][0] + vl1["box_style"][4]
                        ):
            ...
            if bad_sequence:
                ...
                if (
                    0
                    < (block["box_style"][0] - prev_blk_bottom)
                    < 2 * block["box_style"][4]
                ):
                    bad_sequence = False
            no_more_table_rows = (
                no_more_non_table_rows or bad_sequence or italic_center
            ) and table_start_idx != -1
            # print("no_more_table_rows: ", no_more_table_rows, bad_sequence, no_more_non_table_rows)
            if (
                table_start_idx != -1
                and not is_table_row
                and self.detect_block_center_aligned(block, enable_width_check=False)
                and prev_block
                and prev_block["block_type"] != "table_row"
            ):
                ...
                if (
                    blk_has_parenthesized_hdr
                    or (
                        len(block["visual_lines"]) > 1
                        and block["visual_lines"][0]["text"].strip().startswith("(")
                        and block["visual_lines"][-1]["text"].strip().endswith(")")
                    )
                    or self.detect_block_center_aligned(
                        prev_block, enable_width_check=False
                    )
                ):
            ...
            if table_start_idx != -1 and n_rows > 2:
                for tblock in organized_blocks[table_start_idx:table_end_idx]:
                    ...
                    if (
                        block_vl["line_style"]
                        == tblock["visual_lines"][0]["line_style"]
                        and n_rows <= 4
                    ):
            ...
            if (
                not no_more_table_rows
                and non_table_row_count > 0
                and table_start_idx != -1
                and not is_table_row
                and n_rows > 2
                and not last_block_in_page
                and check_again
            ):
                if left_most_vl:
                    ...
                    intersection_points = list(
                        range(
                            max(int(prev_box[1]), int(curr_box[1])),
                    ...
                    if (
                        (
                            intersection_points
                            and not block_vl["line_style"] == left_most_vl["line_style"]
                        )
                        or block_box[1] < min_left
                        or curr_box[3] > 0.6 * self.page_width
                        or block_box[3] > 0.6 * self.page_width
                    ):
                        ...
                        space_bw_curr_and_prev = abs(
                            block["box_style"][0] - prev_blk_bottom
                        )
                        if organized_blocks[-1]["page_idx"] != block["page_idx"]:
                            space_bw_curr_and_prev = 0
                        if (
                            space_bw_curr_and_prev > 2.5 * median_gap_bw_rows
                            and space_bw_curr_and_prev > 1.5 * max_gap_bw_rows
                        ):
                            no_more_table_rows = True
                        if (
                            no_more_table_rows
                            and idx < len(self.blocks) - 1
                            and self.blocks[idx + 1]["block_type"] == "table_row"
                            and prev_block["block_type"] == "table_row"
                            and n_rows <= 4
                        ):
                            ...
                            and self.blocks[idx + 1]["block_type"] == "table_row"
                            and prev_table_row
                            and prev_table_row["visual_lines"][0]["line_style"]
                            != block_vl["line_style"]
                        ):
                            ...
                            align_count, prev_count, _ = (
                                table_parser.get_alignment_count(
                                    prev_table_row, self.blocks[idx + 1]
                                )
                            )
                            space_bw_curr_and_prev_row = abs(
                                block["box_style"][0]
                                - (
                            ...
                            if (
                                align_count < prev_count
                                and len(gap_bw_actual_rows) > 0
                                and space_bw_curr_and_prev_row
                                > 1.5 * max(gap_bw_actual_rows)
                                and space_bw_curr_and_prev_row > 1.5 * max(gap_bw_rows)
                            ):
                                no_more_table_rows = True
                if not no_more_table_rows and len(gap_bw_rows) > 0:
                    ...
                    if space_bw_curr_and_prev < 1.5 * median_gap_bw_rows:
                        align_count, _, _ = table_parser.get_alignment_count(
                            prev_block, block
                        )
                        if align_count:
            ...
            if (
                ...
                if (
                    Doc.check_line_between_box_styles(
                        prev_block["box_style"], block["box_style"], svg_page_tags[0]
                    )
                    and 0 < space_bw_curr_and_prev < prev_block_style[4]
                ):
                    no_more_table_rows = False
                    align_count, _, _ = table_parser.get_alignment_count(
                        prev_block, block
            ...
            if prev_block:
                prev_block_table_row = prev_block["block_type"] == "table_row"
                if (
                    not prev_block_table_row
                    and prev_block.get("last_block_in_page", False)
                    and len(organized_blocks) > 1
                ):
                    prev_block_table_row = (
                        organized_blocks[-2]["block_type"] == "table_row"
                    )
                ...
                too_much_space = (
                    bottom_space > table_end_space_threshold
                    and top_space > table_end_space_threshold
                )
                if line_style in self.line_style_space_stats:
                    too_much_space = (
                        ...
                        > 1.4
                        * self.line_style_space_stats[line_style]["most_frequent_space"]
                        and curr_top > prev_bottom
                        and curr_top - prev_bottom > 1.5 * block["box_style"][4]
                    )
                    if (
                        space_bw_table_rows
                        and (curr_top - prev_bottom) > 2 * space_bw_table_rows
                        and (curr_top - prev_bottom)
                        > 2
                        * self.line_style_space_stats[line_style]["most_frequent_space"]
                    ):
                ...
                smaller_font = (
                    line_style[2] < prev_line_style[2] if prev_line_style else False
                )
                larger_font = (
                    line_style[2] > prev_line_style[2] if prev_line_style else False
                )
                different_style = (
                    line_style != prev_line_style if prev_line_style else False
                )
                has_more_cols = (
                    len(block["visual_lines"]) - len(prev_block["visual_lines"]) > 0
                )
                ...
                if table_start_idx > 0:  # and 'merged_block' not in prev_block:
                    ...
                    def determine_align_break(align_count, prev_count, align_pos):
                        ...
                        if prev_count > 1:
                            align_break = (
                                align_count < 2
                                or max(align_pos) <= align_count
                                and align_count / prev_count < align_threshold
                            )
                        else:
                    ...
                    if is_table_row:
                        ...
                        if prev_block_table_row:
                            align_count, prev_count, align_pos = (
                                table_parser.get_alignment_count(prev_block, block)
                            )
                            page_change_alignment_style_check = (
                                align_count == prev_count
                                or (
                                    align_count > prev_count
                                    and block["block_class"]
                                    == prev_block["block_class"]
                                    and 0 in align_pos
                                )
                            )
                            alignment_break = determine_align_break(
                                align_count, prev_count, align_pos
                            )
                            pattern_broke = False
                            if (
                                not alignment_break
                                and vhu.find_num_cols(block)[0] > prev_count
                                and n_rows > 2
                            ):
                                all_rows_has_same_cols = True
                                for tb_row in organized_blocks[
                                    table_start_idx : table_end_idx - 1
                                ]:
                            ...
                            if (
                                alignment_break
                                and not align_count
                                and table_row_with_max_cols
                                and not pattern_broke
                            ):
                            ...
                            if (
                                alignment_break
                                and prev_block.get("merged_block", False)
                                and idx + 1 < len(self.blocks)
                                and self.blocks[idx + 1]
                                and Doc.have_y_overlap(
                                    block, self.blocks[idx + 1], check_class=True
                                )
                            ):
                            ...
                            prev_table_row
                            and prev_table_row["block_type"] == "table_row"
                            and (
                                curr_top
                                > prev_table_row["box_style"][0]
                                + prev_table_row["box_style"][4]
                                or prev_table_row["page_idx"] != block["page_idx"]
                            )
                        ):
                            align_count, prev_count, align_pos = (
                                table_parser.get_alignment_count(prev_table_row, block)
                            )
                            align_threshold = 2 / 3
                            if prev_count > 1:
                                ...
                                alignment_break = (
                                    align_count < 2
                                    or align_count / prev_count < align_threshold
                                )
                                if (
                                    alignment_break
                                    and align_count == vhu.find_num_cols(block)[0]
                                ):
                                    ...
                                    alignment_break
                                    and align_count / len(block["visual_lines"])
                                    >= align_threshold
                                    and 0 < block_idx - prev_table_row["block_idx"] < 3
                                ):
                        ...
                        if (
                            len(organized_blocks[table_start_idx:]) == 1
                            and alignment_break
                        ...
                        and prev_block["page_idx"] != block["page_idx"]
                    ):
                        ...
                        page_change_alignment_style_check = (
                            page_change_alignment_style_check
                            and Doc.check_line_between_box_styles(
                                prev_box_style,
                                block["box_style"],
                                svg_page_tags[0],
                                x_axis_relaxed=True,
                ...
                page_change = new_page and (
                    n_table_cols > 2
                    or (n_table_cols == 2 and not is_table_row and not aligned_para)
                )

                # different_section = too_much_space and (alignment_break or smaller_font or larger_font)
                different_section = (
                    too_much_space or prev_too_much_space
                ) and alignment_break
                ...
                if (
                    not different_section
                    and too_much_space
                    and double_space_bw_table_rows
                    and block["block_class"] != prev_block["block_class"]
                ):
            ...
            new_table = (
                (different_section and is_table_row)
                or (
                    is_table_row
                    and page_change
                    and not page_change_alignment_style_check
                )
                or alignment_break
            )
            ...
            different_section = (
                different_section or two_col_table_end
            )  # or different_column
            # if different_section:
            ...
            if table_ended:
                ...
                if table_start_idx != table_end_idx:
                    ...
                    #     organized_blocks.pop()
                    #     idx = idx - 1
                    block_idx, footer_count = self.build_table(
                        block_idx, organized_blocks, table_start_idx, table_end_idx
                    )
                    all_vls_has_parenthesized_hdr = False
                    last_table_row_blk = organized_blocks[0 - footer_count - 1]
                    if (
                        last_table_row_blk
                        and last_table_row_blk["block_type"] == "table_row"
                    ):
                    ...
                    if (
                        no_more_table_rows or new_table
                    ) and all_vls_has_parenthesized_hdr:
                        # Break down the 2 row table to header & para
                        if (
                            organized_blocks[0 - footer_count - 1]["block_type"]
                            == organized_blocks[0 - footer_count - 2]["block_type"]
                            == "table_row"
                            and (table_end_idx - footer_count - table_start_idx) == 2
                        ):
                            last_block = organized_blocks[0 - footer_count - 1]
                            prev_to_last_block = organized_blocks[0 - footer_count - 2]
                            if (
                                vhu.find_num_cols(last_block)[0]
                                == vhu.find_num_cols(prev_to_last_block)[0]
                            ):
                                ...
                                for v_idx, vls in enumerate(last_block["visual_lines"]):
                                    h_blk = self.make_block(
                                        [vls], "header", start_block_idx
                                    )
                                    h_blk["block_class"] = prev_to_last_block[
                                        "block_class"
                                    ]
                                    organized_blocks.insert(start_block_idx, h_blk)
                                    start_block_idx += 1
                                    cell_blk = self.make_block(
                                        [prev_to_last_block["visual_lines"][v_idx]],
                ...
                if new_table:
                    ...
                    if table_parser.TABLE_DEBUG:
                        print(
                            f"\ttable changed: align: {align_count}, "
                    ...
                    if table_parser.TABLE_DEBUG or table_parser.TABLE_BOUNDS_DEBUG:
                        print(
                            f"\ttable ended: section: {different_section}, no rows: {no_more_table_rows}, "
            ...
            if block["block_type"] == "list_item":
                block["list_type"] = Doc.get_list_item_subtype(block)
            if (
                self.detect_block_center_aligned(block, enable_width_check=False)
                and parenthesized_hdr_pattern.search(block["block_text"]) is not None
                and block["block_type"] != "header_modified_to_para"
            ):
                ...
                if (
                    block["block_type"] != "table_row"
                    and curr_block["block_type"] != "table_row"
                ):
                    # Change the block_class and block_idx
                    block["block_class"], curr_block["block_class"] = (
                        curr_block["block_class"],
                        block["block_class"],
                    )
                    block["block_idx"], curr_block["block_idx"] = (
                ...
                if curr_block["block_type"] == "table_row":
                    if (
                        not table_row_with_max_cols
                        or vhu.find_num_cols(curr_block)[0]
                        > vhu.find_num_cols(table_row_with_max_cols)[0]
                    ):
        ...
        if table_start_idx != table_end_idx:
            self.build_table(
                block_idx, organized_blocks, table_start_idx, table_end_idx + 1
            )
        ...
        while i < len(organized_blocks):
            ...
            if a_block["block_type"] in [
                "para",
                "list_item",
                "header_modified_to_para",
            ]:
                possible_header = (
                    a_block["visual_lines"][-1]["line_style"] in self.header_styles
                )
                ...
                if (
                    a_block.get("block_class", "")
                    and a_block["block_class"]
                    != a_block["visual_lines"][-1].get("class", "")
                    and self.class_line_styles[a_block["block_class"]]
                    not in self.header_styles
                ):
                ...
                for vls in a_block["visual_lines"]:
                    ...
                    if lp:
                        all_vls_incomplete_line = (
                            all_vls_incomplete_line and lp["incomplete_line"]
                        )
                        ...
                        break
                bulleted_list_item = (
                    "list_type" in a_block
                    and a_block["list_type"] in line_parser.list_types.values()
                )
                should_be_header = (
                    possible_header
                    ...
                    and (
                        line_props.word_count < 16
                        or (
                            line_props.word_count < 20
                            and line_props.incomplete_line
                            and not line_props.continuing_line
                            and all_vls_incomplete_line
                            and not line_props.last_continuing_char
                        )
                    )
                ...
                if possible_header and not should_be_header:
                    ...
                    for b in para_to_header_blocks_font_decider:
                        if (
                            b["visual_lines"][-1]["line_style"]
                            == a_block["visual_lines"][-1]["line_style"]
                            and b.get("orig_block_type", "")
                            and b["orig_block_type"] == "para"
                            and ends_with_sentence_delimiter_pattern.search(
                                b["block_text"]
                            )
                            is not None
                ...
                if (
                    should_be_header
                    and a_block["block_type"] != "header_modified_to_para"
                ):
                ...
                if a_block["block_type"] == "list_item" and should_be_list:
                    ...
                    for m in floating_number_pattern.finditer(
                        line_w_o_num, re.MULTILINE
                    ):
                        t = m.group()
                        t = t.replace(".", "__dot__")
                    ...
                    if (
                        lp_first_sent.is_header
                        and lp_first_sent.is_reference_author_name
                    ) or lp_first_sent.is_header_without_comma:
                        diff_in_len = len(a_block["block_text"].split()) - len(
                            line_props.line_without_number.split()
                        )
                        if diff_in_len > 0:
                            vl_word_classes = []
                            ...
                            for v in a_block["visual_lines"]:
                                ...
                                if len(vl_word_classes) > (
                                    len(first_sent.split()) + diff_in_len + 1
                                ):
                                    break
                            if vl_word_classes and len(vl_word_classes) > (
                                len(first_sent.split()) + diff_in_len
                            ):
                                prev_class_name = vl_word_classes[
                                    len(first_sent.split()) + diff_in_len - 1
                                ]
                                curr_class_name = vl_word_classes[
                                    len(first_sent.split()) + diff_in_len
                                ]
                                if self.has_smaller_or_lighter_header_font(
                                    prev_class_name, curr_class_name
                                ):
                                    # Check whether the previous word is of higher font than the current one.
                                    different_fonts = True
                    if (
                        lp_first_sent.is_header or lp_first_sent.is_header_without_comma
                    ) and (
                        not lp_first_sent.is_reference_author_name or different_fonts
                    ):
                        a_block["block_type"] = "header"
                        a_block["list_type"] = Doc.get_list_item_subtype(a_block)
                        para_sentence = line_props.line_without_number[
                            len(first_sent) + 1 :
                        ].strip()
                        para_sentence = start_punct_pattern.sub(
                            "", para_sentence
                        ).strip()
                    ...
                    if (
                        lp_first_sent.is_header
                        and lp_first_sent.is_header_without_comma
                        and lp_first_sent.is_reference_author_name
                    ):
                        ...
                        for rev_blk in organized_blocks[i::-1]:
                            if rev_blk[
                                "block_type"
                            ] == "header" and self.has_same_or_bigger_font(
                                rev_blk["block_class"], a_block["block_class"]
                            ):
                    ...
                    should_be_list
                    or (
                        a_block["block_text"].lower().startswith("section ")
                        and a_block["block_text"][0].isupper()
                        and section_generic_pattern.search(
                            a_block["block_text"].lower()
                        )
                        is not None
                    )
                ):
                    if should_be_list:
                        first_sent = line_props.line_without_number.split(".")[0]
                        title = a_block["block_text"][
                            : a_block["block_text"].find(first_sent) + len(first_sent)
                        ]
                        para_sentence = line_props.line_without_number[
                            len(first_sent) + 1 :
                        ].strip()
                        ...
                        if (
                            lp_first_sent.is_header
                            and len(a_block["visual_lines"]) > 1
                            and para_sentence
                        ):
                            add_para_block = True
                        elif (
                            lp_first_sent.is_header
                            and (len(a_block["visual_lines"]) == 1 or not para_sentence)
                            and not lp_first_sent
                        ):
                            a_block["block_type"] = "header"
                        ...
                        match = section_generic_pattern.match(
                            a_block["block_text"].lower()
                        )
                        if match:
                            section_header = True
                            first_sent = a_block["block_text"][
                                : len(match.group())
                            ].strip()
                            title = first_sent
                            a_block["block_type"] = "header"
                            a_block["block_sents"] = [first_sent]
                            para_sentence = a_block["block_text"][
                                len(first_sent) + 1 :
                            ].strip()
                            a_block["block_text"] = title
                ...
                if para_sentence and add_para_block:
                    ...
                    if section_header or len(a_block["visual_lines"][0]["text"]) < len(
                        first_sent
                    ):
                        consider_para_vls = True
                        temp_first_sent = first_sent.replace(
                        ...
                        for vl_idx, vl in enumerate(
                            a_block["visual_lines"][block_start_vl_idx:]
                        ):
                            vl["text"] = start_punct_pattern.sub(
                                "", vl["text"].strip()
                            ).strip()
                            all_done = True
                            ...
                            for word in first_sent_words:
                                ...
                                if not vl["text"]:
                                    ...
                                    temp_first_sent = temp_first_sent.replace(
                                        word, ""
                                    ).strip()
                                    vl["text"] = vl["text"][len(word) :].strip()
                                    vl["word_classes"] = (
                                        vl["word_classes"][1:]
                                        if len(vl["word_classes"]) > 1
                                        else vl["word_classes"]
                                    )
                            if all_done:
                                vl["text"] = start_punct_pattern.sub(
                                    "", vl["text"].strip()
                                ...
                                start_idx = (
                                    vl_idx + 1 if block_start_vl_idx > 0 else vl_idx
                                )
                                if not vl["text"]:
                                    start_idx += 1
                                para_vls = a_block["visual_lines"][start_idx:]
                                break
                    elif len(header_block["block_text"]) + 1 < len(
                        a_block["visual_lines"][0]["text"]
                    ):
                        ...
                        a_block["visual_lines"][0]["text"] = (
                            a_block["visual_lines"][0]["text"]
                            .replace(header_block["block_text"], "")
                            .strip()
                        )
                        a_block["visual_lines"][0]["text"] = start_punct_pattern.sub(
                            "", a_block["visual_lines"][0]["text"].strip()
                        ).strip()
                        len_header_words = len(header_block["block_text"].split())
                        if len_header_words < len(
                            a_block["visual_lines"][0]["word_classes"]
                        ):
                            a_block["visual_lines"][0]["word_classes"] = a_block[
                                "visual_lines"
                            ][0]["word_classes"][
                                len(header_block["block_text"].split()) :
                            ]
                        else:
                            a_block["visual_lines"][0]["word_classes"] = a_block[
                                "visual_lines"
                    ...
                    para_block["visual_lines"] = (
                        para_vls if consider_para_vls else a_block["visual_lines"][1:]
                    )
                    ...
                    while j < len(organized_blocks):
                        ...
                        if (
                            "header_block_idx" in organized_blocks[j]
                            and organized_blocks[j]["header_block_idx"] > idx
                        ):
                    ...
                    if len(para_block["visual_lines"]):
                        ...
                        if para_props.numbered_line:
                            ...
                            para_block["list_type"] = Doc.get_list_item_subtype(
                                para_block
                            )
    ...
    def build_table(self, block_idx, organized_blocks, table_start_idx, table_end_idx):
        footer_count, footers = self.get_table_footers(
        ...
        if table_parser.TABLE_DEBUG:
            ...
            print(
                "\ttable start idx",
                table_start_idx,
                organized_blocks[table_start_idx]["block_text"],
            )
            print("\ttable end idx", table_end_idx)
            print("\tblock_idx", block_idx)
        if table_end_idx - footer_count - table_start_idx > 1:
            block_idx, table_end_idx = self.make_table_with_footers(
                block_idx,
                footer_count,
                footers,
                organized_blocks,
            ...
            tr_block["block_type"] = get_block_type(
                False, False, tr_block["block_text"]
            )[0]
    ...
    def parse_special_lists(self, blocks):
        ...
        def check_special_line(block):
            for line_idx, line in enumerate(block["visual_lines"]):
                ...
                if not line["line_parser"]["noun_chunks"]:
                    return False
                noun_chunk = non_alphanumeric_pattern.sub(
                    " ", line["line_parser"]["noun_chunks"][0]
                ).strip()
                text = non_alphanumeric_pattern.sub(" ", line["text"]).strip()
                upper_check = all([token[0].isupper() for token in text.split()])
                bolded_text = (
                    line["line_style"][1] == "bold" or line["line_style"][3] > 400
                )
        ...
        while i < len(blocks):
            ...
            if block["block_type"] == "list_item" and not block.get("list_type", None):
                block["list_type"] = Doc.get_list_item_subtype(block)
            if (
                block["block_type"] == "table_row" or "one_row_table" in block
            ) and not is_table:
                is_table = True
                is_special = True
                table_start_idx = i
            ...
            if (
                ...
                if (
                    center_italic_idx < len(blocks)
                    and self.detect_block_center_aligned(blocks[center_italic_idx])
                    and block["page_idx"] != blocks[center_italic_idx]["page_idx"]
                ):
                    center_italic_idx = i
                else:
    ...
    def calculate_block_bounds_from_vls(self, blocks):
        ...
        bottom = (
            blocks[-1]["visual_lines"][0]["box_style"][0]
            + blocks[-1]["visual_lines"][0]["box_style"][4]
        )
        left = 1000000
        right = -1
        for block in blocks:
    ...
    def make_table_with_footers(
        ...
        possible_table_blocks = organized_blocks[
            table_start_idx : table_end_idx - footer_count
        ]
        ...
        if table_start_idx > 0:
            ...
            same_class = (
                prev_block["visual_lines"][-1]["line_style"]
                == curr_top_block["visual_lines"][-1]["line_style"]
            )
            ...
            for row_idx in range(
                table_start_idx, min(table_end_idx, table_start_idx + 3)
            ):
                # next 3 blocks
                # print(organized_blocks[row_idx]["block_text"])
                align_count, prev_count, align_pos = table_parser.get_alignment_count(
                    prev_block, organized_blocks[row_idx]
                )
            ...
            if (
                ...
                prev_block["visual_lines"], _, _ = self.merge_vls_if_needed(
                    prev_block["visual_lines"]
                )
                prev_block["block_type"] = "table_row"
    ...
    def get_table_footers(
        ...
        while end_idx > table_start_idx:
            ...
            if (
                table_block["block_type"] != "table_row"
                and "aligned_para" not in table_block
            ):
    ...
    def collapse_group(self, group_buf, group_class_name):
        ...
        for idx, item in enumerate(group_buf):
            buf_text = (
                buf_text
                + self.check_add_space_btw_texts(buf_text, item["text"])
                + item["text"]
            )
    ...
    def compress_blocks(self):
        for block in self.blocks:
            ...
            if "orig_vls" in block:
                del block["orig_vls"]
            if (
                "effective_header" in block
                and "visual_lines" in block["effective_header"]
            ):
    ...
    def merge_line_info(first, second, remove_space=False):
        ...
        if first["box_style"][0] != second["box_style"][0]:
            if first["box_style"][0] < second["box_style"][0]:
                height = (second["box_style"][0] + second["box_style"][4]) - first[
                    "box_style"
                ][0]
            else:
                height = (first["box_style"][0] + first["box_style"][4]) - second[
                    "box_style"
                ][0]
        ...
        combined_text = (
            first["text"]
            + Doc.check_add_space_btw_texts(first["text"], second["text"])
            + second["text"]
        )
        ...
        combined_block = {
            ...
            "line_style": (
                first["line_style"] if len(first["text"]) > 1 else second["line_style"]
            ),
            "text": combined_text,
            ...
            "lp_line": lp_line,
            "line_parser": lp_line.to_json(),
            "changed": (first["changed"] if "changed" in first else False)
            or (second["changed"] if "changed" in second else False),
        }
        return combined_block

    @staticmethod
    def get_location_key(box_style, line_text):
        line_text = only_text_pattern.sub(
            "", line_text
        )  # re.sub(r"[^a-zA-Z]+", "", line_text)
        line_text = roman_only_pattern.sub("", line_text)
        return LocationKey(int(box_style[0]), int(box_style[1]), line_text)
    ...
    def find_true_header_footers(
        ...
        def get_hf_keys_with_same_text(
            hf: dict, hf_key: namedtuple, check_page_pattern, page_pat_val
        ):
            m_keys = []
            m_pages = []
            for key in hf.keys():
                ...
                if (
                    abs(key[0] - hf_key[0]) < 20
                    and abs(key[1] - hf_key[1]) < 20
                    and (key[2] == hf_key[2] or key[2] in page_pat_val)
                ):
                    m_keys.append(key)
            ...
            return (
                m_keys,
                sorted(list(dict.fromkeys(m_pages))) if len(m_pages) > 1 else m_pages,
                True,
                page_pat_val,
            )
        ...
        for hf_key in hf.keys():
            ...
            if not key_with_text_added:
                matched_keys, list_of_pages, check_page_pattern, page_pattern_list = (
                    get_hf_keys_with_same_text(
                        hf, hf_key, check_page_pattern, page_pattern_list
                    )
                )
            else:
                matched_keys = [hf_key]
            ...
            if num_matched_pages > 1 and (
                np.mean(diff_of_pages) <= 2
                or (
                    np.median(diff_of_pages) <= 2
                    and is_footer
                    and hf_key.text != ""
                    and is_footer_key_below_page_num_footer(hf_key, result, hf)
                )
                or (
                    np.median(diff_of_pages) <= 2
                    and Counter(diff_of_pages).most_common()[0][0] <= 2
                    and np.percentile(diff_of_pages, 75) <= 2
                )
            ):
                if (
                    num_matched_pages > 0.5 * num_pages
                    or hf_key.text == ""
                    or (num_matched_pages > 0.2 * num_pages and max(list_of_pages) > 2)
                    or (num_matched_pages > 5 and max(list_of_pages) > 2)
                ):
    ...
    def calc_block_span(block):
        ...
        for vl in block_vl[1:]:
            ...
            if vl_box[0] > prev_vl_box[0]:
                n_lines = n_lines + 1
                height = (
                    height + vl_box[4] + (vl_box[0] - (prev_vl_box[0] + prev_vl_box[4]))
                )
                prev_vl_box = vl_box
        ...
        box_style = BoxStyle(top, min_left, max_right, max_right - min_left, height)
        # print("--")
    ...
    def merge_vls(vl_merge_buf):
        ...
        vl_box_style = BoxStyle(
            vl_top, vl_left, vl_right, vl_right - vl_left, vl_height
        )
    ...
    def merge_blocks(self, blocks):
        ...
        if len(blocks) == 1:
            return blocks[0]
        for block in blocks:
            any_block_table_row = any_block_table_row or (
                block["block_type"] == "table_row"
            )
            if same_top and prev_block:
                all_blocks_to_the_right = (
                    all_blocks_to_the_right
                    and prev_block["box_style"][2] < block["box_style"][1]
                )
            merged_text = (
                merged_text
                + self.check_add_space_btw_texts(merged_text, block["block_text"])
                + block["block_text"]
            )
            ...
            for vl in block_vls[1:]:
                same_top = (
                    same_top and abs(prev_vl["box_style"][0] - vl["box_style"][0]) < 0.1
                )
                if MERGE_DEBUG:
                    print("\tvl ", vl["text"])
            ...
            if "merged_block" not in block:
                vls[merge_vls_idx:], _, _ = self.merge_vls_if_needed(
                    vls[merge_vls_idx:], block["block_type"] == "table_row"
                )
            merge_vls_idx = len(vls)
            prev_block = block

        box_style = BoxStyle(min_top, left, left + width, width, max_height)

        lp_line = line_parser.Line(blocks[0]["block_text"])
        block_type = (
            "list_item"
            if (lp_line.is_list_item or lp_line.numbered_line)
            else "table_row"
        )
        if (
            not any_block_table_row
            and block_type != "list_item"
            and same_top
            and not all_blocks_to_the_right
        ):
            block_type = blocks[0]["block_type"]
        elif (
            not any_block_table_row
            and block_type != "list_item"
            and all_blocks_to_the_right
            and len(blocks) == 2
            and blocks[0]["box_style"][3] >= 0.2 * self.page_width
            and abs(blocks[0]["box_style"][3] - blocks[1]["box_style"][3])
            <= blocks[0]["visual_lines"][0]["line_style"][5] * 3
        ):
            prev_line_ends_with_line_delim = (
                ends_with_sentence_delimiter_pattern.search(blocks[0]["block_text"])
                is not None
            )
    ...
    def determine_block_type(self, block):
        ...
        if len(vls) > 1:
            first_vl = vls[0]
            second_vl = vls[1]
            prev_line_style = self.class_line_styles[first_vl["word_classes"][-1]]
            curr_line_style = self.class_line_styles[second_vl["word_classes"][0]]
            normal_gap = max(
                prev_line_style[5],
                curr_line_style[5],
            )
            gap = second_vl["box_style"][1] - first_vl["box_style"][2]
            if gap > table_col_threshold * normal_gap:
                if (
                    first_vl["text"].startswith("(")
    ...
    def get_gaps_from_vls(self, curr_vl, prev_vl):
        ...
        if cline_style in self.line_style_word_stats:
            ...
            if line_word_stats["is_justified"]:
                is_justified = True
                if (
                    line_word_stats["avg"] == line_word_stats["median"] == 1.0
                ) and line_word_stats["std"] == 0.0:
    ...
    def filter_list_of_bbox(list_of_bbox, **kwargs):
        ...
        matches = filter(
            lambda bbox: all([bbox[k] == v for k, v in kwargs.items()]), list_of_bbox
        )
    ...
    def create_new_vl_group_for_sections(self, result_list, buf_texts, block_types):
        ...
        if len(set(result_list[0][0]["word_classes"])) == 1 and (
            len(result_list[0]) > 1 and len(set(result_list[0][1]["word_classes"])) == 1
        ):
            if (
                result_list[0][0]["word_classes"][0]
                != result_list[0][1]["word_classes"][0]
            ):
                block_types = ["header_modified", "para"]
                buf_texts = [
                    result_list[0][0]["text"]
                    + self.check_add_space_btw_texts(
                        result_list[0][0]["text"], result_list[0][1]["text"]
                    )
                ...
                len(result_list[0][0]["word_classes"]) == 1
                and len(result_list[0]) > 2
                and result_list[0][0]["word_classes"][0]
                != result_list[0][2]["word_classes"][0]
            ):
                split_idx = -1
                for idx, vl in enumerate(result_list[0][3:]):
                    if (
                        result_list[0][idx + 3]["word_classes"][0]
                        != result_list[0][2]["word_classes"][0]
                    ):
                        split_idx = idx + 3
                        break
            ...
            if len(set(result_list[0][0]["word_classes"])) > 1:
                ...
                len(result_list[0]) > 1
                and len(set(result_list[0][1]["word_classes"])) > 1
            ):
            ...
            if split_idx > -1:
                ...
                if ptag_idx > -1:
                    all_p = self.pages[result_list[0][split_idx]["page_idx"]].find_all(
                        "p"
                    )
                    ...
                    if prefix_vls:
                        prefix_vls["text"] += " " + " ".join(
                            p_tag.text.split()[:diff_idx]
                        )
                        right = float(
                            input_style["word-end-positions"][diff_idx - 1].split(",")[
                                0
                            ]
                        )
                        box_style = BoxStyle(
                        ...
                        result_list[0][split_idx]["word_classes"] = word_classes[
                            diff_idx:
                        ]
                        result_list[0][split_idx]["text"] = " ".join(
                            p_tag.text.split()[diff_idx:]
                        )
                        if len(block_types) <= 2:
                            ...
                            buf_texts = [
                                prefix_vls["text"],
                                " ".join(
                                    result_list[0][split_idx]["text"].split()[diff_idx:]
                                ),
                            ]
                        ...
                        left = float(
                            input_style["word-start-positions"][0].split(",")[0]
                        )
                        right = float(
                            input_style["word-end-positions"][diff_idx - 1].split(",")[
                                0
                        ...
                        if len(block_types) <= 1:
                            ...
                            if (
                                split_idx == 0
                                and len(result_list[0][split_idx]["word_classes"]) >= 3
                                and diff_idx == 2
                                ...
                                buf_texts = [
                                    line_info["text"],
                                    " ".join(
                                        result_list[0][split_idx]["text"].split()[
                                            diff_idx:
                                        ]
                                    ),
                                ]
        ...
        for blk_idx, blk in enumerate(self.blocks):
            if (
                (
                    blk["block_type"] == "para"
                    or (
                        blk["block_type"] == "list_item"
                        and blk.get("list_type", "NA") == "letter"
                        and blk["block_text"][0].isupper()
                    )
                )
                and ends_with_sentence_delimiter_pattern.search(blk["block_text"])
                is None
                and not blk.get("is_row_group", False)
                and len(blk["visual_lines"]) > 1
            ):
                if blk["block_type"] == "list_item":
                    if len(temp_blocks):
                        if temp_blocks[-1]["block_type"] == "list_item" or (
                            temp_blocks[-1]["block_type"] != "list_item"
                            and blk["block_text"][0] == "A"
                        ):
                ...
                for vl_group in same_line_vl_group:
                    ...
                    if line_props.is_header and line_props.noun_chunks:
                        ...
                        translated_str = last_line_text.translate(
                            str.maketrans("", "", string.punctuation)
                        )
                        if (
                            translated_str == noun_chunk_str
                            or (
                                len(noun_chunk_str.split())
                                / (line_props.word_count - line_props.stop_word_count)
                            )
                            > 0.75
                        ):
    ...
    def merge_para_blocks(self):
        ...
        for blk_idx, blk in enumerate(self.blocks):
            if (
                ...
                and (
                    temp_blocks[-1]["block_class"] == blk["block_class"]
                    or temp_blocks[-1]["visual_lines"][-1]["word_classes"][-1]
                    == blk["visual_lines"][0]["word_classes"][0]
                )
                and blk["page_idx"] == temp_blocks[-1]["page_idx"]
                and ends_with_sentence_delimiter_pattern.search(
                    temp_blocks[-1]["block_text"]
                )
                is None
                and not blk.get("is_row_group", False)
                and (
                    blk["box_style"][0]
                    - (
                        temp_blocks[-1]["box_style"][0]
                        + temp_blocks[-1]["box_style"][4]
                    )
                    <= blk["visual_lines"][0]["line_style"][2]
                    or temp_blocks[-1]["visual_lines"][-1]["line_parser"].get(
                ...
                merged_block = {
                    ...
                    "block_text": merged_text,
                    "merged_block": True,
                    "visual_lines": temp_blocks[-1]["visual_lines"]
                    + blk["visual_lines"],
                ...
                is None
                and (
                    blk["page_idx"] != temp_blocks[-1]["page_idx"]
                    or (
                        blk["page_idx"] == temp_blocks[-1]["page_idx"]
                        and len(temp_blocks) > 1
                        and temp_blocks[-1]["page_idx"] != temp_blocks[-2]["page_idx"]
                    )
                )
            ):
                ...
                if (
                    ...
                    and len(temp_blocks) > 1
                    and temp_blocks[-1]["page_idx"] != temp_blocks[-2]["page_idx"]
                ) and temp_blocks[-1]["block_type"] != "header":
                    rev_start_idx = -2
                for t_blk_idx, t_blk in enumerate(temp_blocks[rev_start_idx::-1]):
                    if (
                        t_blk["block_class"] == blk["block_class"]
                        ...
                        if (
                            ...
                            prev_temp_idx = (
                                len(temp_blocks) - abs(rev_start_idx) - t_blk_idx
                            )
                        break
                    elif (
                        temp_blocks[rev_start_idx]["page_idx"] != t_blk["page_idx"]
                    ):  # Check only last page
                        break
                    elif (
                        t_blk["block_type"] == "header"
                    ):  # Break on the first header encountered
                        break
                if (
                    prev_same_class_block
                    and prev_temp_idx >= 0
                    and (
                        blk["visual_lines"][0]["line_parser"].get(
                            "continuing_line", False
                        )
                        or prev_same_class_block["visual_lines"][-1]["line_parser"].get(
                    ...
                    merged_block = {
                        ...
                        "block_text": merged_text,
                        "merged_block": True,
                        "visual_lines": prev_same_class_block["visual_lines"]
                        + blk["visual_lines"],
                        "block_class": blk["block_class"],
                        "block_sents": sent_tokenize(merged_text),
                    }
                ...
                and blk["page_idx"] == temp_blocks[-1]["page_idx"]
                and (
                    blk["visual_lines"][0]["line_parser"].get("continuing_line", False)
                    or (
                ...
                if (
                    (
                        ...
                        or (
                            ...
                            and (
                                blk["page_idx"] <= 2
                                or blk["visual_lines"][0]["line_parser"].get(
                                    "first_word", ""
                                )
                                in line_parser.conjunction_list + ["as"]
                            )
                        )
                    )
                    and temp_blocks[-1]["box_style"][0]
                    + temp_blocks[-1]["box_style"][4]
                    < blk["box_style"][0]
                    ...
                    for t_blk_idx, t_blk in enumerate(temp_blocks[rev_start_idx::-1]):
                        if (
                            ...
                            break
                if prev_same_class_block and prev_temp_idx >= 0:
                    merged_text = prev_same_class_block["block_text"]
                    merged_text = (
                        merged_text
                        + self.check_add_space_btw_texts(merged_text, blk["block_text"])
                    ...
                    merged_block_class = (
                        prev_same_class_block["block_class"]
                        if len(prev_same_class_block["block_text"].split())
                        > len(blk["block_text"].split())
                ...
                is None
                and not blk.get("is_row_group", False)
                and temp_blocks[-1]["visual_lines"][-1]["line_parser"].get(
                    "last_word_is_co_ordinate_conjunction", False
                )
            ):
    ...
    def merge_header_blocks(self):
        ...
        for blk_idx, blk in enumerate(self.blocks):
            ...
            if (
                ...
                and (
                    centre_aligned_blk
                    and self.detect_block_center_aligned(temp_blocks[-1])
                    or temp_blocks[-1]["box_style"][1] == blk["box_style"][1]
                    or temp_blocks[-1]["visual_lines"][-1]["line_parser"].get(
                        "last_word_is_co_ordinate_conjunction", False
                    )
                )
                ...
                translated_str = blk["block_text"].translate(
                    str.maketrans("", "", string.punctuation)
                )
                ...
                if json_rec["noun_chunks"]:
                    ...
                    if (
                        ...
                        or (
                            len(noun_chunk_str.split())
                            / (json_rec["word_count"] - json_rec["stop_word_count"])
                        )
                        > 0.75
                    ):
                        name_decider = True
    ...
    def merge_ooo_list_para_blocks(self):
        ...
        for blk_idx, blk in enumerate(self.blocks):
            if (
                ...
                temp_blocks[-1]["visual_lines"] = (
                    temp_blocks[-1]["visual_lines"] + blk["visual_lines"]
                )
    ...
    def merge_center_aligned_header_para_blocks(self):
        ...
        for blk_idx, blk in enumerate(self.blocks):
            ...
            if len(temp_blocks):
                last_temp_block_centre_aligned = self.detect_block_center_aligned(
                    temp_blocks[-1]
                )
            if (
                len(temp_blocks)
                and (
                    temp_blocks[-1]["block_type"] in ["header", "para"]
                    and blk["block_type"] in ["header", "para"]
                )
                and blk["page_idx"] == temp_blocks[-1]["page_idx"] == 0
                and (
                    (centre_aligned_blk and last_temp_block_centre_aligned)
                    or (
                        centre_aligned_blk
                        and self.detect_block_center_aligned(self.blocks[blk_idx - 1])
                    )
                    or (
                        self.detect_block_center_aligned(blk, False)
                        and last_temp_block_centre_aligned
                    )
                    or temp_blocks[-1]["visual_lines"][-1]["line_parser"].get(
                        "last_word_is_co_ordinate_conjunction", False
                    )
    ...
    def check_add_space_btw_texts(left_text, right_text):
        ...
        if left_text and right_text:
            if left_text[-1] in ["(", "[", "{", "“"]:
                ...
                return delim
            elif left_text[-1] in ["'", '"', "“"] and any(
                (c in right_text) for c in ["'", '"', "”"]
            ):
                return delim
            else:
                delim = " "
    ...
    def soft_line_style_check(line_info, prev_line_info):
        ...
        prev_line_style = prev_line_info["line_style"]
        if (
            line_style[0] == prev_line_style[0]
            ...
            line_style[0] == prev_line_style[0]
            and line_style[1] in ["normal", "italic"]
            and prev_line_style[1] in ["normal", "italic"]
            and line_style[2] == prev_line_style[2]
            and line_style[3] == prev_line_style[3]
            and line_style[5] == prev_line_style[5]
        ):
    ...
    def has_smaller_or_lighter_header_font(self, prev_class_name, new_class_name):
        ...
        has_smaller_or_lighter_header_font = cur_blk_line_style[
            2
        ] < prev_blk_line_style[2] or (
            (cur_blk_line_style[2] == prev_blk_line_style[2])
            and cur_blk_line_style[3] < prev_blk_line_style[3]
        )
    ...
    def has_same_or_bigger_font(self, prev_class_name, new_class_name):
        prev_blk_line_style = self.class_line_styles[prev_class_name]
        cur_blk_line_style = self.class_line_styles[new_class_name]
        has_same_or_bigger_font = (
            cur_blk_line_style[2] >= prev_blk_line_style[2]
            and not 0 <= cur_blk_line_style[2] - prev_blk_line_style[2] < 0.75
        ) or (
            (cur_blk_line_style[2] == prev_blk_line_style[2])
            and cur_blk_line_style[3] >= prev_blk_line_style[3]
        )
    ...
    def svg_line_operations(lines_list, x1, y1, x2, y2, style):
        if lines_list:
            ...
            for line in lines_list:
                if (
                    abs(x1 - line["x1"]) < 1.0
                    and abs(y1 - line["y1"]) < 1.0
                    and abs(x2 - line["x2"]) < 1.0
                    and abs(y2 - line["y2"]) < 1.0
                ):
                    do_add_to_list = False
                    break
                elif (
                    (abs(x2 - line["x1"]) < 1.0 or abs(x1 - line["x2"]) < 1.0)
                    and abs(y1 - y2) < 1.0
                    and abs(line["y1"] - line["y2"]) < 1.0
                    and abs(y1 - line["y1"]) < 1.0
                ):
                    ...
                    (abs(y2 - line["y1"]) < 1.0 or abs(y1 - line["y2"]) < 1.0)
                    and abs(x1 - x2) < 1.0
                    and abs(line["x1"] - line["x2"]) < 1.0
                    and abs(x1 - line["x1"]) < 1.0
                ):
    ...
    def remove_duplicate_svg_tags(
        ...
        for svg_child in svg_children:
            if (
                svg_child.name == "line"
                and svg_child.get("x1", None)
                and svg_child.get("y1", None)
                and svg_child.get("x2", None)
                and svg_child.get("y2", None)
            ):
                ...
                svg_child.name == "rect"
                and svg_child.get("x", None)
                and svg_child.get("y", None)
                and svg_child.get("height", None)
                and svg_child.get("width", None)
            ):
                if float(svg_child["height"]) > 1.0:
                    # Discard rectangles that are almost the size of the page.
                    if not (
                        float(svg_child["height"]) >= 0.8 * svg_height
                        and float(svg_child["width"]) >= 0.8 * svg_width
                    ):
                    ...
                    style = svg_child.get("style", "")
                    lines_list = Doc.svg_line_operations(
                        lines_list, x1, y1, x2, y2, style
                    )
        ...
        if prev_blk_box_style and curr_blk_box_style and lines_tag_list:
            ...
            for line in lines_tag_list:
                ...
                if (
                    ...
                    and float(line["x1"]) <= left1 <= float(line["x2"])
                    and float(line["x1"]) <= left2 <= float(line["x2"])
                    and float(line["x1"]) < right1 <= float(line["x2"])
                    and float(line["x1"]) < right2 <= float(line["x2"])
                ):
                    if check_gap:
                        if (
                            abs(abs(float(line["y1"]) - bottom1) - abs(top2 - float(line["y1"])))
                            < 2.0
                        ):
                            ret_val = True
                    else:
                    ...
                    x_axis_relaxed
                    and bottom1 <= float(line["y1"]) <= (top2 + 2.0)
                    and float(line["x1"]) <= left1 < float(line["x2"])
                    and float(line["x1"]) < right1 <= float(line["x2"])
                ):
                    if check_gap:
    ...
    def check_block_within_svg_tags(self, block, prev_block):
        ...
        if len(self.page_svg_tags) > block["page_idx"]:
            ...
            for rect in rect_svg_tags:
                ...
                if (
                    rect_top <= box_style[0] <= rect_bottom
                    and rect_left <= box_style[1] <= rect_right
                    and box_style[0] + box_style[4] <= rect_bottom
                    and box_style[2] <= rect_right
                ):
                    ret_val = True
                    break
        return ret_val
