...
class OrderFixer:
    ...
    def group_by_left(self, blocks):
        blocks_sorted_by_left = sorted(
            blocks,
            key=lambda blk: (
                blk["visual_lines"][0]["box_style"][1],
                blk["visual_lines"][0]["box_style"][0],
            ),
        )
        if REORDER_DEBUG:
        ...
        def calc_c_blok_bound(c_blk):
            ...
            non_table_row_blocks: list = [
                blk for blk in c_blk["blocks"] if blk["block_type"] != "table_row"
            ]
            if not non_table_row_blocks:
                return (0, 0, 0, 0), non_table_row_blocks
            # Sort them by y.
            non_table_row_blocks.sort(
                key=lambda blk: blk["visual_lines"][0]["box_style"][0]
            )
            top = non_table_row_blocks[0]["visual_lines"][0]["box_style"][0]
            bottom = (
                non_table_row_blocks[-1]["visual_lines"][0]["box_style"][0]
        ...
        def check_within_cb_bounds(blk, cb_bound, top_sorted_blocks):
            ...
            if nearest_top:
                if (
                    b_top - nearest_top
                ) > 35:  # If we are too far off from the last point
                    return False
        ...
        cnt_table_rows = sum(
            map(lambda blk: blk["block_type"] == "table_row", first_column["blocks"])
        )
        cnt_table_row_vls = sum(
            map(
                lambda blk: (
                    len(blk["visual_lines"]) if blk["block_type"] == "table_row" else 0
                ),
                first_column["blocks"],
            )
        )
        first_column_vls = sum(
            map(lambda blk: len(blk["visual_lines"]), first_column["blocks"])
        )
        for block in first_column["blocks"]:
            if (
                len(contiguous_blocks) > 1
                and block["visual_lines"][-1]["box_style"][2]
                > contiguous_blocks[1]["left_range"][1]
            ):
                if (
                    block["block_type"] == "table_row"
                    and (cnt_table_row_vls / first_column_vls) < 0.40
                    and (cnt_table_rows / len(first_column["blocks"])) < 0.40
                    and not check_within_cb_bounds(block, bounds, non_table_row_blocks)
                ):
                    if REORDER_DEBUG:
            ...
            if len(buf_blocks) > 0:
                ...
                if blk["block_type"] == last_blk["block_type"] == "table_row":
                    if (
                        0
                        <= blk["visual_lines"][0]["box_style"][0]
                        - last_blk["visual_lines"][0]["box_style"][0]
                        <= 70
                    ):
                        return True
                    else:
    ...
    def insert_out_of_order_block(self, oob, main_block):
            ...
            if cond:
                ...
                if gb_top == ob_top or abs(gb_top - ob_top) < 0.1:
                    ...
                    if (oob["box_style"][0] + oob["box_style"][4]) == (
                        main_block[idx]["box_style"][0]
                        + main_block[idx]["box_style"][4]
                ...
                gb_top < ob_top
                and gb_left > ob_right
                and (
                    (
                        oob["visual_lines"][-1]["box_style"][0]
                        + oob["visual_lines"][-1]["box_style"][4]
                    )
                    <= (
                        gb["visual_lines"][-1]["box_style"][0]
                        + gb["visual_lines"][-1]["box_style"][4]
                    )
                )
            ):
                main_block.insert(idx, oob)
        ...
        if not location_found:
            if (
                main_block[0]["visual_lines"][0]["page_idx"]
                == oob["visual_lines"][0]["page_idx"]
            ):
                if REORDER_DEBUG:
    ...
    def reorder_two_column_layout(self):
        ...
        for idx, cb in enumerate(self.contiguous_blocks):
            if REORDER_DEBUG:
                print(
                    f"contiguous block with length {len(cb)} and ratio {self.ratios[idx]} .. ",
        ...
        if len(ratios_to_act) == 2:
            ...
            if ratios_to_act[0] == ratios_to_act[1]:
                ...
                next_main_blk_idx = 1
            if (
                np.argmax(ratios_to_act) == 0
            ):  # main_block is the first contiguous_blocks
                reordered_blocks.extend(self.contiguous_blocks[next_main_blk_idx])
            else:
                if (
                    self.contiguous_blocks[next_main_blk_idx][0]["box_style"][0]
                    < reordered_blocks[-1]["box_style"][0]
                ):
    ...
    def reorder_multi_column_layout(self):
                        ...
                        if ob_in_mb and len(cb["blocks"]) > 0:
                            if REORDER_DEBUG:
                                print(
                                    ...
                                    oob["block_text"],
                                    " in ",
                                    contiguous_blocks[cb_idx]["blocks"][0][
                                        "block_text"
                                    ],
                                )
        ...
        def cb_compare(cb_1, cb_2):
            ...
            if self.doc.have_y_overlap(cb_1["blocks"][0], cb_2["blocks"][0]) or list(
                range(
                    max(int(cb_1_blk_box[0]), int(cb_2_blk_box[0])),
                    min(
                        int(cb_1_blk_box[0] + cb_1_blk_box[4]),
                        int(cb_2_blk_box[0] + cb_2_blk_box[4]),
                    )
                    + 1,
                )
            ):
    ...
    def order_two_contiguous_blocks(self, contiguous_blocks, ratios):
        def comparator():
            def blk_compare(blk1, blk2):
                ...
                if list(
                    range(
                        max(int(cb_1_box[0]), int(cb_2_box[0])),
                        min(
                            int(cb_1_last_box[0] + cb_1_last_box[4]),
                            int(cb_2_last_box[0] + cb_2_last_box[4]),
                        )
                        + 1,
                    )
                ):
        ...
        page_style = list(
            self.doc.page_styles[primary_cb[-1]["visual_lines"][0]["page_idx"]]
        )
        ...
        for blk_idx, blk in enumerate(primary_cb[:-1]):
            if blk["box_style"][0] < bottom_2:
                ...
                if (
                    blk["box_style"][0]
                    <= blk_to_check_style[0]
                    <= primary_cb[blk_idx + 1]["box_style"][0]
                ):
        ...
        if secondary_cb_in_bw_primary:
            ...
            for cb in contiguous_blocks:
                if not len(reordered_blocks):
                    reordered_blocks = (
                        sorted(cb, key=cmp_to_key(cmp))
                        if not page_style[3].get("probable_multi_column", False)
                        else cb
                    )
                else:
                    reordered_blocks.extend(
                        sorted(cb, key=cmp_to_key(cmp))
                        if not page_style[3].get("probable_multi_column", False)
                        else cb
                    )
    ...
    def reorder(self):
        ...
        for b in self.blocks_to_reorder:
            ...
            if (
                curr_top >= prev_top
                or abs(curr_top - prev_top) < TOP_THRESHOLD
                or (len(block_buf) > 0 and block_buf[-1]["page_idx"] != b["page_idx"])
                or (
                    prev_block
                    and 0
                    <= b["visual_lines"][-1]["box_style"][0]
                    - prev_block["visual_lines"][-1]["box_style"][0]
                    < TOP_THRESHOLD
                )
            ):
                if len(temp_storage):
                ...
                for b_idx, b_buf in enumerate(block_buf):
                    ...
                    if (
                        abs(curr_b_buf_top - curr_top) < TOP_THRESHOLD
                        and curr_b_buf_box[2] < curr_box[1]
                        and len(b_buf["visual_lines"]) > 3
                        and b_buf["block_type"] != "table_row"
                    ):
                        for b_temp_buf in block_buf[b_idx + 1 :]:
                            add_to_temp = True
                            if (
                                b_temp_buf["visual_lines"][0]["box_style"][2]
                                > curr_box[1]
                                or b_temp_buf["block_type"] == "table_row"
                            ):
                                add_to_temp = False
                                break
                if (
                    not add_to_temp
                    and len(block_buf) > 0
                    and block_buf[-1]["block_type"] != "table_row"
                ):
                    prev_last_vl_box_style = block_buf[-1]["visual_lines"][-1][
                        "box_style"
                    ]
                    prev_first_vl_box_style = block_buf[-1]["visual_lines"][0][
                        "box_style"
                    ]
                    if (
                        prev_first_vl_box_style[0] > prev_last_vl_box_style[0]
                        and prev_first_vl_box_style[2] < prev_last_vl_box_style[1]
                        and (
                            abs(
                                (prev_last_vl_box_style[0] + prev_last_vl_box_style[4])
                                - curr_top
                            )
                            < TOP_THRESHOLD
                            or (
                                curr_top
                                > (
                                    prev_last_vl_box_style[0]
                                    + prev_last_vl_box_style[4]
                                )
                            )
                        )
                    ):
        ...
        if probable_multi_column:
            page_style = list(
                self.doc.page_styles[
                    self.page_blocks[-1]["visual_lines"][0]["page_idx"]
                ]
            )
        ...
        if two_column_oo_blocks or (
            (len([i for i in ratios if i >= 0.05]) < 3) and len(ratios) > 2
        ):
            reordered_blocks, skipped_blocks = self.reorder_two_column_layout()
        elif multi_column_oo_blocks:
            block_dim = [
                find_cb_left_and_right(cb)
                for cb_idx, cb in enumerate(self.contiguous_blocks)
            ...
            reordered_blocks = self.order_two_contiguous_blocks(
                self.contiguous_blocks, ratios
            )
        else:
            reordered_blocks = self.blocks_to_reorder
