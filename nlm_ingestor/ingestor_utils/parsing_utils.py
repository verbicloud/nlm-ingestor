...
def check_possible_table(prev_block, curr_block):
    ...
    """
    span1 = (
        prev_block["visual_lines"][0]["box_style"][1],
        prev_block["visual_lines"][-1]["box_style"][2],
    )
    span2 = (
        curr_block["visual_lines"][0]["box_style"][1],
        curr_block["visual_lines"][-1]["box_style"][2],
    )
    ...
    if round(overlap) == 1:
        ...
        if len(prev_block_space_dict) > 1 and curr_block_space_dict:
            ...
            if gap_threshold == 0:
                return False, [], []
            new_block_children, new_visual_lines = format_to_tr_block(
                prev_block, gap_threshold
            )
            if compare_centroids(curr_block, new_block_children) or compare_right_align(
...
def format_to_tr_block(prev_block, gap_threshold):
    ...
    for child in prev_block["visual_lines"][1:]:
        ...
        if gap_threshold <= round(child_x1 - prev_child_x2):
            new_child_block = nlm_ingestor.ingestor.visual_ingestor.Doc.merge_vls(
                block_buff
            )
            new_visual_lines.append(new_child_block)
            new_block_children.append(
                {
                    "text": block_text,
                    "centroid": get_centroid(
                        block_buff[0]["box_style"][1], block_buff[-1]["box_style"][2]
                    ),
